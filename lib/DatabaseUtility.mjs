...
export default class DatabaseUtility {
    ...
    static async getUserFitbitDailyDateAndGoalDuringPeriodById(fitbitId, startDateString, endDateString, goalType, recentLimit){
        console.log(`${this.name} getUserFitbitDailyDateAndGoalDuringPeriodById fitbitId: ${fitbitId}, start: ${startDateString}, end:${endDateString}, goalType: ${goalType}`);
        ...
        const minsList = await DatabaseUtility.getUserFitbitWearingMinutesPerDayListDuringPeriod(fitbitId, startDate, endDate);

        // now, start from the ending, find three days that are valid and find the corresponding daily goals
        for(let i = minsList.length - 1; i >= 0; i--){
            let dateAndGoal = {};
            const record = recordList[i];

            if(minsList[i] >= wearingLowerBoundMinutes){
                if(record.content.summary != undefined){
                    dateAndGoal =  {
                        dateTime: record.dateTime,
                        goal: record.content.summary.goals[goalType]
                    };
                }
                else{
                    console.log(`${this.name} getUserFitbitDailyDateAndGoalDuringPeriodById recoord (no content summary): ${JSON.stringify(record, null, 2)}`);
                    dateAndGoal =  {
                        dateTime: record.dateTime,
                        goal: -1
                    };
                }
                console.log(`Add date goal: ${JSON.stringify(dateAndGoal)}`);
                dateGoalList.push(dateAndGoal);
            }
            

            if (recentLimit !== undefined && dateGoalList.length >= recentLimit) {
                break;
            }
        }

        console.log(`${this.name} getUserFitbitDailyDateAndGoalDuringPeriodById dateGoalList: ${dateGoalList}`);
