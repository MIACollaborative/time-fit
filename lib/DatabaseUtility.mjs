...
export default class DatabaseUtility {
    ...
    static async queryAndStoreFitbitIntradayHeartRateAtTargetDateForUser(userInfo, targetDate, insertToDB=true, numOfDays=1){
        console.log(`${this.name}.queryAndStoreFitbitIntradayHeartRateAtTargetDateForUser: ${userInfo.username}, ${targetDate}, numOfDays: ${numOfDays}`);

        // .toISODate() -> what will be used by FitbitHelper
        console.log(`${this.name}.queryAndStoreFitbitIntradayHeartRateAtTargetDateForUser: targetDate.toISODate:, ${targetDate.toISODate()}`);
        ...
        // { value: "failed", data: inspect(error.response.data) };
        let validateTokenResult = await DatabaseUtility.ensureTokenValidForUser(userInfo, true, 30 * 60);
        console.log(`${this.name}.queryAndStoreFitbitIntradayHeartRateAtTargetDateForUser: token validation: ${validateTokenResult.value}`);
        ...
        for(let i = 0; i < numOfDays; i++){
            ...
            let activityResult = await FitbitHelper.getIntradayHeartRateBetweenDateRangeForFitbitId(updatedUserInfo.fitbitId, updatedUserInfo.accessToken, curDate, curDate)
            .then((responseData) => {
              console.log(
                `FitbitHelper.queryAndStoreFitbitIntradayHeartRateAtTargetDateForUser: ${JSON.stringify(
                  responseData
                )}`
              );
            ...
            console.log(`${this.name}.queryAndStoreFitbitIntradayHeartRateAtTargetDateForUser [${curDate.toFormat('yyyy-MM-dd')}]: query result: ${activityResult.value}`);
            ...
            if(insertToDB == true && activityResult.value == "success"){
                ...
                // now insert the data
                // To Do: decide the schema
                let dataType = "heartrate-intraday";
    ...
    static async queryAndStoreFitbitIntradayHeartRateDataByFitbitUpdate(fitbitUpdate, insertToDB=true){
        console.log(`${this.name}.queryAndStoreFitbitIntradayHeartRateDataByFitbitUpdate: ${JSON.stringify(fitbitUpdate)}`);
        ...
        console.log(`${this.name}.queryAndStoreFitbitIntradayHeartRateDataByFitbitUpdate userInfo: ${JSON.stringify(userInfo)}`);


        const summaryResult = await DatabaseUtility.queryAndStoreFitbitIntradayHeartRateAtTargetDateForUser(userInfo, targetDate, insertToDB, 1);
