...
export default class DatabaseUtility {
    ...
    static async replacePlaceholderFromMessage(message, userInfo, surveyLink){
        ...
        if( result.message.includes("[goal]")){
            ...
            const endDateString = DateTime.now().minus({days: 1}).toFormat('yyyy-MM-dd');
            const startDateString = endDateString.minus({days: 30}).toFormat('yyyy-MM-dd');

            const wearingDateGoalList = await DatabaseUtility.getUserFitbitDailyGoalsForWearingDaysDuringPeriodById(userInfo.fitbitId, startDateString, endDateString, "steps", 60 * 8, 3);
            ...
            if(wearingDateGoalList.length > 0){
                // calculate average step goals
                averageStepGoal = wearingDateGoalList.reduce((total, next) => total + next.goal, 0) / wearingDateGoalList.length;
            }
            else{
                // grab the curren daily step goal
                averageStepGoal = (await FitbitHelper.getActivityGoalsForFitbitID(fitbitID, accessToken, "daily"))["goals"]["steps"];
            }

            const roundedStepGoal = Math.floor(averageStepGoal * randomMultiplier/100) * 100;

            result.message = voca.replaceAll(result.message, '[goal]', `${roundedStepGoal}`); 
            result.surveyReplaced = true;
        }
    ...
    static async getUserFitbitDailyStepsForWearingDaysDuringPeriodById(fitbitId, startDateString, endDateString, goalType, wearingLowerBoundMinutes, recentLimit){
        ...
        const wearingDateStepsList = dateStepsList.filter((dateSteps) => {
            //console.log(`[${dateGoal.dateTime}][${dateGoal.wearingMinutes >= wearingLowerBoundMinutes}] ${dateGoal.wearingMinutes}`);
