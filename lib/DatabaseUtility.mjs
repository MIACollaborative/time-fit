...
export default class DatabaseUtility {
    ...
    static async queryAndStoreFitbitActivitySummaryAtTargetDateForUser(userInfo, targetDate){
        ...
        // now query the data
        const activityResult = await FitbitHelper.getActvitySummaryForFitbitId(updatedUserInfo.fitbitId, updatedUserInfo.accessToken, targetDate)
        .then((responseData) => {
          console.log(
        ...
        if(activityResult.value == "success"){
            resultData = activityResult.data;

            // now insert the data
            // To Do: decide the schema
            let dataType = "activity-summary";
            let compositeId = GeneralUtility.generateCompositeIDForFitbitUpdate([updatedUserInfo.fitbitId,dataType, resultData.activities[0].startDate]);
