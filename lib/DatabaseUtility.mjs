...
export default class DatabaseUtility {
    ...
    static async queryAndStoreFitbitDataByFitbitUpdate(fitbitUpdate){
        console.log(`${this.name}.queryAndStoreFitbitDataByFitbitUpdate: ${fitbitUpdate}`);

        let resultData = {};

        // get the user first
        let userInfo = prisma.users.findFirst({
            where: {
                fitbitId: fitbitUpdate.ownerId
            }
        });

        let targetDate = fitbitUpdate.date;

        const summaryResult = await DatabaseUtility.queryAndStoreFitbitActivitySummaryAtTargetDateForUser(userInfo, targetDate, true, 1);
