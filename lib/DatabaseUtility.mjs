...
export default class DatabaseUtility {
    ...
    static async ensureTokenValidForUser(userInfo){
        ...
        .then((responseData) => {
            ...
            */
            ...
            await DatabaseUtility.updateToken(userInfo.hash, newAccessToken, newRefreshToken, userInfo);
    ...
    static async createSubscriptionForUser(userInfo){
        ...
        let newSubscriptionId = currentSubCount + 1;
        ...
        */
        ...
        let activitySubscriptionResult = await FitbitHelper.createSubscriptionForFitbitId(updatedUserInfo.fitbitId, "activites", newSubscriptionId, updatedUserInfo.accessToken);

        await prisma.fitbit_subscription.create({
            data: activitySubscriptionResult
        });

        resultList.push(activitySubscriptionResult);

        // second, for "userRevokedAccess"
        let revokedSubscriptionResult = await FitbitHelper.createSubscriptionForFitbitId(updatedUserInfo.fitbitId, "userRevokedAccess", newSubscriptionId, updatedUserInfo.accessToken);

        await prisma.fitbit_subscription.create({
            data: revokedSubscriptionResult
        });

        resultList.push(revokedSubscriptionResult);
