...
export default class DatabaseUtility {
            ...
            if(v.startsWith(match, 'survey_link_from_tasks', 1)){
                ...
                for(let j = 0; j < taskLabelList.length; j++){
                    ...
                    let filteredTaskLogList = taskLogList.filter((taskLog) => {
                        return taskLog.randomizationResult.theChoice.action.surveyLink.length > 0;
                    });
    ...
    static async findTaskLogWithActionTypeDuringPeriod(actionType, startDateTime, endDateTime, limit=0){
        ...
        itemList = itemList.filter((itemInfo) => {
            return itemInfo["randomizationResult"]["theChoice"]["action"]["type"] == actionType;
        });
    ...
    static async shouldFitbitUpdateBeProcessed(fitbitUpdate){
        console.log(`${this.name}.shouldFitbitUpdateBeProcessed: ${JSON.stringify(fitbitUpdate)}`);
        ...
        userInfo = JSON.parse(JSON.stringify(aUser, replacer));

        console.log(`${this.name}.shouldFitbitUpdateBeProcessed user: [${userInfo.username}]--------------`);

        let dateString = fitbitUpdate.date;
        let targetDate = DateTime.fromISO(dateString);

        console.log(`${this.name}.shouldFitbitUpdateBeProcessed userInfo: ${JSON.stringify(userInfo)}`);
        ...
        // result
        let validInterval = Interval.fromDateTimes(startDate, endDate.plus({milliseconds: 1}));
    ...
    static async queryAndStoreFitbitIntradayDataAtTargetDateForUser(userInfo, fitbitIntradayDataType, targetDate, insertToDB=true, numOfDays=1, suppressTokenValidation=false){
        ...
        for(let i = 0; i < numOfDays; i++){
            ...
            .then((responseData) => {
              console.log(
                `FitbitHelper.getIntradayDataBetweenDateRangeForFitbitId: ${JSON.stringify(
                  responseData
                )}`
              );
