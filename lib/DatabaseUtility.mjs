...
export default class DatabaseUtility {
    ...
    static async replacePlaceholderFromMessage(message, userInfo, surveyLink){
        ...
        if( result.message.includes("[7-day-average-steps]")){
            ...
            result.message = voca.replaceAll(result.message, '[7-day-average-steps]', value); 
            result.surveyReplaced = true;
        }
        ...
        for(let i = 0; i < matchList.length; i++){
            ...
            if(v.startsWith(match, 'response', 1)){
                ...
                let responseList = await DatabaseUtility.findSurveyResponoseDuringPeriod(surveyId, DateTime.now().minus({years: 1}), DateTime.utc(), 1);
    ...
    static async findSurveyResponoseDuringPeriod(surveyId, startDate, endDate, limit=0){
