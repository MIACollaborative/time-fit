...
export default class GeneralUtility {
    ...
    static taskList = [
                    ...
                    },
                    {
                        ...
                        criteria: {
                            ...
                            period: {
                                ...
                                start: {
                                    reference: "today",
                                    offset: { type: "minus", value: { days: 7 } } // checks for wearing adherence the last 7 days
                                },
                                // reference:
                                // now: current time
                                // today: start of today (00:00:00 am)
                    ...
                    },
                    {
                        ...
                        criteria: {
                            start: {
                                ...
                                /*
                                reference: "now", //ONLY FOR TESTING - CHANGE BACK TO JOINATDATE
                                offset: { type: "plus", value: { days: 0 } }
                                */
                            }
                        }
        ...
        {
            // Send adherence reminder for wearing Fitbit for +8hours IF non-worn days for 2 consecutive days, x3 days at wakeup+1hour
            label: "TESTING for fitbit wearing check",// we need to manually make sure that it is unique
            enabled: false,
            priority: 1, // 1 (highest) ~ 100 (lowest)
            participantIndependent: false,
            ignoreTimezone: false,
                    ...
                    },
                    {
                        ...
                        criteria: {
                            ...
                            period: {
                                ...
                                start: {
                                    reference: "today",
                                    offset: { type: "minus", value: { days: 6 } } // checks for wearing adherence the last 6 days
                                },
                                // reference:
                                // now: current time
                                // today: start of today (00:00:00 am)
        ...
        {
            label: "baseline remaining tasks message", //Send a reminder message to complete all remaining baseline tasks to be activated to intervention phase on Monday
            enabled: false,
            priority: 1, // 1 (highest) ~ 100 (lowest)
            participantIndependent: false,
            ignoreTimezone: false,
    ...
    static extractActionToString(theChoice) {
        let result = `${theChoice.type} -`;
        let excludeNameList = ["type"];

        Object.keys(theChoice).forEach((propertyName) => {
            if(!excludeNameList.includes(propertyName)){
                if(theChoice[theChoice] != undefined && theChoice[theChoice] != ""){
                    result = result + ` ${propertyName}:${theChoice[theChoice]}`;
                }
            }
        });
