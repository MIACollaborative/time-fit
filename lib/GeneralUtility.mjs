...
export default class GeneralUtility {
    ...
    static taskList = [
                        ...
                        criteria: {
                            ...
                            period: {
                                start: {
                                    ...
                                    offset: { type: "minus", value: { hours: 1 } }
                                },
                                end: {
                                    // reference:
                                    // now: current time
        ...
        },
        {
            ...
            checkPoint: {
                ...
                reference: {
                    ...
                    type: "fixed",
                    value: "00:01 AM" // (if preference) (wakeupTime, bedTime, createdAt) -> need to support wakeupTime
        ...
        },
        {
            ...
            checkPoint: {
                ...
                repeat: {
                    interval: { minutes: 30 }, // every x (5) minutes
                    range: {
                        // after: starting from that reference, before, strating befoore that reference
                        before: {
                            // will execute within distance (100 mins) prior to the reference point
        ...
        },
        {
            ...
            checkPoint: {
                ...
                repeat: {
                    interval: { minutes: 2 }, // every x (5) minutes
                    range: {
                        // after: starting from that reference, before, strating befoore that reference
                        /*
                        before: {
        ...
        {
            label: "TEST_morning gif",// remove task after testing
            enabled: false,
            priority: 1, // 1 (highest) ~ 100 (lowest)
            participantIndependent: false,
            preActivationLogging: false,
                ...
                outcome: [
                    {
                        ...
                        action: {
                            type: "messageLabel", // messageLabel, or messageGroup
                            messageLabel: "gif-m_35", //messageLabel, only matter if the type is messageLabel
                            messageGroup: "gif-m", // "nongif-m", // messageGroup, only matter if the type is messageGroup
                            avoidHistory: true, // if we want to minimize the chance of sending the same message to the same user in a short window
                        }
                    },
        ...
        },
        {
            ...
            preCondition: {
                ...
                conditionList: [
                    ...
                    {
                        // Check if participant completed any of the planning surveys
                        type: "surveyFilledByThisPerson",
                        criteria: {
                            // Id list: list of Qualtrics survey Ids to check
                            idList: ["SV_bBoOhje0dSNbZgq", "SV_cACIS909SMXMUp8", "SV_6QJa9e00C4gywQu"],
                            // first planning survey: SV_bBoOhje0dSNbZgq
                            // try something new survey: SV_cACIS909SMXMUp8
                            // stay the same survey: SV_6QJa9e00C4gywQu
                            ...
                            period: {
                                ...
                                start: {
                                    reference: "today",
                                    offset: { type: "minus", value: { days: 3 } } // 3days to account for thursday
                                },
                                // reference:
                                // now: current time
                                // today: start of today (00:00:00 am)
                ...
                outcome: [
                    {
                        ...
                        action: {
                            type: "messageLabel", // messageLabel, or messageGroup
                            messageLabel: "prompt-re_17", //messageLabel, only matter if the type is messageLabel
                            messageGroup: "", // "nongif-m", // messageGroup, only matter if the type is messageGroup
                            avoidHistory: false, // if we want to minimize the chance of sending the same message to k,the same user in a short window
                            surveyType: "", //surveyLabel or surveyLink
                            surveyLink: ""
        ...
        {
            label: "TESTING for messageSentDuringPeriod",// we need to manually make sure that it is unique
            enabled: false,
            priority: 1, // 1 (highest) ~ 100 (lowest)
            participantIndependent: false,
            preActivationLogging: false,
                        ...
                        criteria: {
                            ...
                            period: {
                                ...
                                start: {
                                    ...
                                    // offset, the time that will be added ("plus") or substracted ("minus") from the reference
                                    // Plus 0 hours basically means using the reference point directly
                                    offset: { type: "minus", value: { days: 42 } } // checks for survey completion the last 5 days (not including the day of message)
                                },
    ...
    static generateStartOrEndDateTimeByReference(targetDateTime, userInfo, timeDefinition, startOrEnd = "start") {
        console.log(`${this.name} generateStartOrEndDateTimeByReference: ${targetDateTime}, ${timeDefinition}`);
        let resultDateTime;
    ...
    static generateDateTimeByReference(targetDateTime, userInfo, reference, startEndOfUnit = "no", startEndUnit = "day") {
        console.log(`${this.name} generateDateTimeByReference: ${targetDateTime},${userInfoo.username},${reference}, ${startEndOfUnit},${startEndUnit}`);
        let resultDateTime;
