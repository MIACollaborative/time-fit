...
class MongoDBHelper {
  ...
  static async getDataFromTable(tableName, query={}, options={}){
    ...
    try {
      ...
      const database = await client.db("miwaves").command({ ping: 1 });
      const docs = await client.db("miwaves").collection(tableName);
    ...
    /*
    let resultInfoList = JSON.parse(JSON.stringify(resultList, replacer));
    */
  ...
  static async insertDocListIntoTable(collectionName, docList){
    ...
    try {
      ...
      await client.db("miwaves").command({ ping: 1 });
      ...
      const insertManyresult = await client.db("miwaves").collection(collectionName).insertMany(timedDocList);
  ...
  static async insertOrUpdateOneDocFromTable(tableName, doc, query, options){
    ...
    const existResult = await MongoDBHelper.getDataFromTable(tableName, query, {});
		...
		if(existResult.length > 0){
			...
			console.log(`submitCredentialListByUsername options: ${JSON.stringify(options)}`);
  ...
  static async updateOneDocFromTable(tableName, query, options, upsert=true){
    ...
    //$set: { options["$set"] != undefined?  ...options["$set"], updated_at: DateTime.now().toJSDate() }

    console.log(`options (after): ${JSON.stringify(timedOptions)}`);
