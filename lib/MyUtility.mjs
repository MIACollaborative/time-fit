...
export default class MyUtility {
    ...
    static async findMessageByGroup(gGroupName, avoidHistory=false){

        console.log(`MyUtility.findMessageByGroup: ${gGroupName}`);
    ...
    static replacePlaceholderFromMessage(message, userInfo, surveyLink){
        ...
        if( result.message.includes("[name]")){
            ...
            result.message = result.message.replace("[name]", userInfo.preferredName);
            result.nameReplaced = true;
        }

        if( result.message.includes("<link>")){
            console.log(`MyUtility.replacePlaceholderFromMessage found <link>`);
            result.message = result.message.replace("<link>", surveyLink);
    ...
    static async getUserMessageLabelFromGroupWithLowestFrequency(username, groupName){
        //let resultList = [];

        let frequencyDict = await MyUtility.getUserMessageFromGroupCountDict(username, groupName);

        let frequencyList = Object.keys(frequencyDict).map((key) => {
            return {
                messageLabel: key,
    ...
    static async getUserMessageFromGroupCountDict(username, groupName){
        ...
        messageLabelList.forEach((messageLabel) => {
            if(userMessageCountDict[messageLabel] != undefined){
                resultDict[messageLabel] = userMessageCountDict[messageLabel];
            }
            else{
                resultDict[messageLabel] = 0;
            }
        });
        return resultDict;
    }
