...
export default class TaskExecutor {
    ...
    static async isPreConditionMetForUser(conditionSpec, userInfo, dateTime) {
        ...
        */
        ...
        console.log(`${this.name} isPreConditionMetForUser[${userInfo.username}]: resultList: ${conditionEvaluationResultList}`);
    ...
    static async checkOneConditionForUser(condition, userInfo, dateTime) {
        ...
        switch (condition.type) {
            ...
            case "messageSentDuringPeriod":
                ...
                let taskLogList = await DatabaseUtility.findTaskLogDuringPeriod(startDate, endDate);
                ...
                taskLogList = taskLogList.filter((taskLogInfo) => {
                    return taskLogInfo.username == userInfo.username && taskLogInfo.messageLabel == condition.messageLabel;
                });

                console.log(`messageSentDuringPeriod (${condition.messageLabel}) for [${userInfo.username}] taskLogList.length: ${taskLogList.length}`);

                console.log(`messageSentDuringPeriod (${condition.messageLabel}) for [${userInfo.username}] taskLogList.length > 0: ${taskLogList.length > 0}`);

                result = taskLogList > 0;
                break;
