...
export default class TaskExecutor {
  ...
  static async executeTaskForUserListForDatetime(taskSpec, userList, datetime){
    ...
    for(let i = 0; i < userList.length; i++ ){
        ...
        console.log(`executeTaskForUserList (${userInfo.username}): chanceChoice: ${JSON.stringify(chanceChoice)}`);
        let taskLogObj = {};
        ...
        let compositeResult = await TaskExecutor.executeActionForUser(theAction, userInfo);
        ...
        console.log(`executeTaskForUserList (${userInfo.username}) taskLogObj: ${JSON.stringify(taskLogObj)}`);
  ...
  static async executeActionForUser(theAction, userInfo){
      ...
      switch(theAction.type){
        case "messageLabel":
            ...
            messageInfo = await MyUtility.findMessageByLabel(theAction.messageLabel);
            console.log(`executeActionForUser messageInfo: ${JSON.stringify(messageInfo)}`);

            // for logging
            record.messageLabel = messageInfo.label;

            surveyURL = await MyUtility.extractSurveyLinkFromMessageInfo(messageInfo);

            messageBody = MyUtility.composeUserMessageForTwilio(userInfo, messageInfo, surveyURL);
            ...
            break;
        case "messageGroup":
            messageInfo = await MyUtility.findMessageByGroup(theAction.messageGroup, theAction.avoidHistory, userInfo.username);
            console.log(`executeActionForUser messageInfo: ${JSON.stringify(messageInfo)}`);
