    ...
    static async executeActionForUser(theAction, userInfo, datetime) {
        ...
        switch (theAction.type) {
            ...
            case "processFitbitUpdateForIntraday":
                    let processedUpdateList = await DatabaseUtility.getFitbitUpdateByStatusWithLimit("processed", 1);
    
    
                    console.log(`processedUpdateList: ${JSON.stringify(receprocessedUpdateListtUpdateList, null, 2)}`);
                    ...
                    for(let i = 0; i < cleanUpdateList.length; i++){
                        ...
                        let result = await DatabaseUtility.queryAndStoreFitbitIntradayHeartRateDataByFitbitUpdate(fitbitUpdate, true);
                    ...
                    */
                    ...
                    let successResultOnlyList = resultList.filter((result) => {
                        return result.value == "success";
                    });
                    ...
                    for(let i = 0; i < successResultOnlyList.length; i++){
                        ...
                        const updateOlderList = await DatabaseUtility.updateSameOrOlderFitbitUpdateStatusWithSameSignature(fUpdate, "processed", "processed-intraday");
                    ...
                    record.executionResult = {
                        type: "fitbit-process-update-for-intraday",
                        value: {
                            status: resultStatus,
                            errorMessage: resultErrorMessage,
                            body: resultBody
