...
export default class TaskExecutor {
    ...
    static async executeTaskForUserListForDatetime(taskSpec, userList, datetime) {
        ...
        userList = userList.filter((userInfo) => {
            return TaskExecutor.isPreConditionMetForUser(taskSpec.preCondition, userInfo);
        });
    ...
    static isPreConditionMetForUser(conditionSpec, userInfo, dateTime) {
        ...
        */
        ...
        let conditionEvaluationResultList = conditionList.map((condition) => {
            return TaskExecutor.checkOneConditionForUser(condition, userInfo, dateTime);
        });
        ...
        result = GeneralUtility.reduceBooleanArray(conditionEvaluationResultList, condicionSpec.conditionRelationship);

        return result;
    }

    static checkOneConditionForUser(condition, userInfo, dateTime) {
        console.log(`${this.name} checkOneConditionForUser`);
        let result = true;
        ...
        switch(condition.type){
            ...
            case "surveyFilledByThisPerson":
                ...
                let surveyFillResultList = condition.criteria.idList.map((surveyId) => {
                    return DatabaseUtility.findSurveyResponoseDuringPeriod(surveyId, startDate, endDate).length > 0;
                });
