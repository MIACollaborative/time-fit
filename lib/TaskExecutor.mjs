...
export default class TaskExecutor {
    ...
    static async executeTaskForUserListForDatetime(taskSpec, userList, datetime) {
        ...
        for (let i = 0; i < userList.length; i++) {
            ...
            if (taskSpec.ignoreTimezone == false) {
                ...
                console.log(`isTimezoneSet: user[${userInfo.username}]: ${isTimeZoneSetResult}`);
            ...
            console.log(`isGroupForUser: user[${userInfo.username}]: ${isGroupResult}`);
            ...
            let [isCheckPointResult, checkPointEvaluationRecordList] = TaskExecutor.isCheckPointForUser(taskSpec.checkPoint, userInfo, datetime);
            console.log(`isCheckPointResult: user[${userInfo.username}]: ${isCheckPointResult}`);
    ...
    static async executeActionForUser(theAction, userInfo, datetime) {
        ...
        switch (theAction.type) {
            ...
            case "generateManualFitbitUpdate":
                ...
                try {
                    if (proxyUpdateList.length > 0) {
                        insertProxyUpdateResult = await DatabaseUtility.insertFitbitUpdateList(proxyUpdateList);
                    }

                    resultErrorMessage = `Attempt to insert: ${proxyUpdateList.length}, Insert: ${proxyUpdateList.length > 0 ? insertProxyUpdateResult.count : 0}`;
            ...
            case "processFitbitUpdate":
                ...
                let recentTaskLogWithResultList = recentTaskLogList.filter((taskLog) => {
                ...
                console.log(`recentTaskLogWithResultList.length: ${recentTaskLogWithResultList.length}`);
                ...
                */
                ...
                for (let i = 0; i < filteredUpdateList.length; i++) {
                    ...
                    let shouldProcess = await DatabaseUtility.isFitbitUpdateDateWithinAppropriateScope(fitbitUpdate);
    ...
    static async isPreConditionMetForUser(conditionSpec, userInfo, dateTime) {
        console.log(`${this.name} isPreConditionMetForUser[${this.taskSpec != undefined ? this.taskSpec.label : "[testing] no taskSpec"}] for ${userInfo.username}`);
        ...
        */
        ...
        console.log(`${this.name} isPreConditionMetForUser[${userInfo.username}]: result: ${result}, resultList: ${conditionEvaluationResultList}, resultReocrdList: ${JSON.stringify(evaluationReportList, null, 2)}`);
        ...
        switch (condition.type) {
            ...
            case "hasFitbitUpdateForPersonByDateRange":
                ...
                */
                ...
                if(userInfo.fitbitId != undefined && userInfo.fitbitId.length > 0){
                    updateList = await DatabaseUtility.getUserFitbitUpdateDuringPeriodByIdAndOwnerType(userInfo.fitbitId, startDate, endDate, "user");
                }


                console.log(`${this.name} checkOneConditionForUser type: ${condition.type}: start: ${startDate}, end: ${endDate}, updateList.length: ${updateList.length}`);
            ...
            case "hasHeartRateIntradayMinutesAboveThresholdForPersonByDateRange":
                ...
                */
                ...
                let minsList = [];
                if(userInfo.fitbitId != undefined && userInfo.fitbitId.length > 0){
                    minsList = await DatabaseUtility.getUserFitbitWearingMinutesPerDayListDuringPeriod(userInfo.fitbitId, startDate, endDate);
                }
            ...
            case "hasTaskLogErrorByDateRange":
                ...
                // generic
                let errorList = await DatabaseUtility.getTaskLogWithErrorDuringPeriod(startDate, endDate);

                console.log(`${this.name} checkOneConditionForUser type: ${condition.type}: start: ${startDate}, end: ${endDate}, errorList: ${JSON.stringify(errorList, null, 2)}`);
