...
export default class TaskExecutor {
  ...
  static executeTask(taskSpec, userList){
    ...
    userList = userList.filter((userInfo) => {
        return isGroupForUser(taskSpec.group, userInfo);
    });
    ...
    let theAction = obtainActionWithRandomization(taskSpec.randomization);


    // Step 4: execute the action for each user
    let actionResultList = userList.map((userInfo) => {
        return executeActionForUser(theAction, userInfo);
    });
  ...
  static obtainActionWithRandomization(randomizationSpec){
    ...
    */
    ...
    let theChoice = randomizeSelection(randomization.outcome);
    let theAction = {
        type: "noAction"
    };
  ...
  static executeActionForUser(theAction, userInfo){
  ...
  static isGroupForUser(groupSpec, userInfo){
    if(groupSpec.type == "all"){
        ...
        Object.keys(groupSpec.membership).forEach((groupName) => {
            if(groupSpec.membership.includes(userInfo[groupName])){
                groupMatched = true;
            }
        });
        return groupMatched;
