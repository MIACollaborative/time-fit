      ...
      switch(theAction.type){
        ...
        case "retrieveFitbitData":
            ...
            */
            ...
            let nowData = DateTime.now();
            let targetDate = nowData;
            let dateString;
            if( theAction.retrievalStartDate.reference == "today"){
                ...
                if(theAction.retrievalStartDate.offset.type == "minus"){
                    targetDate = nowData.minus(theAction.retrievalStartDate.offset.value);
                }
                else{
                    targetDate = nowData.plus(theAction.retrievalStartDate.offset.value);
                }

                dateString = targetDate.toFormat('yyyy-MM-dd');
            }

            const summaryResult = await DatabaseUtility.queryAndStoreFitbitActivitySummaryAtTargetDateForUser(userInfo, targetDate, false);
            const heartrateResult = await DatabaseUtility.queryAndStoreFitbitHeartRateAtTargetDateForUser(userInfo, targetDate, false);

            record.executionResult = {
                type: "fitbit",
                value: [summaryResult, heartrateResult]
            };
